...
export function HowItWorks() {
  const steps = [
    {
      ...
      glowColor: 'from-yellow-500/20 to-amber-500/0'
    },
    {
      number: '02',
  ...
  return (
    <div className="space-y-12 py-4">
          ...
          <div key={idx} className="glow-card group">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-4">
                <span className="text-[10px] font-black tracking-widest text-yellow-500 uppercase">Step {step.number}</span>
                <h3 className="text-lg font-bold text-slate-100 group-hover:text-yellow-300 transition-colors duration-200">{step.title}</h3>
                <p className="text-xs text-slate-400 leading-relaxed font-medium">{step.description}</p>
              </div>

              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-slate-950 border border-yellow-500/10 group-hover:border-yellow-500/30 group-hover:shadow-[0_0_15px_rgba(255,210,8,0.1)] transition-all duration-300">
                {step.icon}
              </div>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto bg-slate-950/40 border border-yellow-500/10 rounded-2xl p-6 sm:p-8 space-y-6 text-left">
        ...
        <div className="grid gap-6 sm:grid-cols-3">
          {features.map((feat, idx) => (
            <div key={idx} className="space-y-2 border-r border-yellow-500/10 last:border-0 pr-4 last:pr-0">
              <h4 className="text-sm font-bold text-yellow-400">{feat.title}</h4>
