  ...
  const steps = [
    {
      ...
      icon: <Lock className="h-6 w-6 text-slate-200" />,
      glowColor: 'from-slate-300/20 to-slate-400/0'
    },
    {
      number: '02',
      ...
      icon: <Cpu className="h-6 w-6 text-slate-200" />,
      glowColor: 'from-slate-300/20 to-slate-400/0'
    },
    {
      number: '03',
      ...
      icon: <Key className="h-6 w-6 text-slate-200" />,
      glowColor: 'from-slate-300/20 to-slate-400/0'
    },
    {
      number: '04',
  ...
  return (
    <div className="space-y-12 py-4">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-4">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-slate-300/20 bg-slate-300/5 text-xs font-semibold text-slate-200 shadow-md">
          <HelpCircle className="h-3.5 w-3.5" />
          Technical Walkthrough
        </div>
        <h1 className="text-4xl font-extrabold tracking-tight md:text-5xl font-sans">
          How <span className="bg-gradient-to-r from-white via-yellow-200 to-slate-200 bg-clip-text text-transparent">CipherBallot</span> Works
        </h1>
          ...
          <div key={idx} className="glow-card group">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-4">
                <span className="text-[10px] font-black tracking-widest text-slate-300 uppercase">Step {step.number}</span>
                <h3 className="text-lg font-bold text-slate-100 group-hover:text-white transition-colors duration-200">{step.title}</h3>
                <p className="text-xs text-slate-400 leading-relaxed font-medium">{step.description}</p>
              </div>

              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-slate-950 border border-slate-300/10 group-hover:border-slate-300/30 group-hover:shadow-[0_0_15px_rgba(255,210,8,0.1)] transition-all duration-300">
                {step.icon}
              </div>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto bg-slate-950/40 border border-slate-300/10 rounded-2xl p-6 sm:p-8 space-y-6 text-left">
        ...
        <div className="grid gap-6 sm:grid-cols-3">
          {features.map((feat, idx) => (
            <div key={idx} className="space-y-2 border-r border-slate-300/10 last:border-0 pr-4 last:pr-0">
