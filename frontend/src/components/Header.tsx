...
export function Header({
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-violet-500/10 bg-[#060312]/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        {/* Logo and Tagline */}
        <div className="flex items-center gap-2.5 shrink-0 cursor-pointer" onClick={() => setActiveTab('how-it-works')}>
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-600 shadow-md shadow-violet-600/20">
            <Vote className="h-5 w-5 text-white" />
          </div>
          <div>
            <span className="font-sans text-lg font-black tracking-tight bg-gradient-to-r from-white to-violet-300 bg-clip-text text-transparent">
              CipherBallot
            </span>
          </div>
        </div>
        ...
        {isConnected && (
          <nav className="hidden md:flex items-center gap-1.5 px-1.5 py-1 bg-slate-950/40 rounded-full border border-violet-500/5">
            {navItems.map((item) => (
              <button
                key={item.id}
                ...
                className={`flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-full transition-all duration-200 ${
                  activeTab === item.id
                    ? 'bg-gradient-to-r from-violet-600 to-indigo-600 text-white shadow-sm shadow-violet-500/10'
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {/* Wallet Connection */}
          {isConnected ? (
            <div className="flex items-center rounded-full bg-[#0b081c] border border-violet-500/10 p-1">
              <div className="flex items-center gap-2 px-3 py-1 text-xs text-slate-300 font-semibold">
                ...
                <span className="h-1 w-1 rounded-full bg-violet-500/30"></span>
                <span className="text-violet-200">{truncateAddress(address)}</span>
              </div>
              <button
                onClick={disconnectWallet}
                className="rounded-full px-3 py-1 text-xs font-bold text-slate-400 hover:text-white hover:bg-violet-950/40 transition duration-150"
              >
                Disconnect
              </button>
            </div>
      ...
      {isConnected && (
        <nav className="flex md:hidden items-center justify-around border-t border-violet-500/5 bg-slate-950/60 py-2">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={`flex flex-col items-center gap-1 px-3 py-1 text-[10px] font-semibold transition-all duration-200 ${
                activeTab === item.id ? 'text-violet-400 font-bold' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              {item.icon}
