...
export function Header({
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-slate-300/10 bg-[#080808]/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2.5 shrink-0 cursor-pointer" onClick={() => setActiveTab('landing')}>
          ...
          <div>
            <span className="font-sans text-lg font-black tracking-tight bg-gradient-to-r from-white to-white bg-clip-text text-transparent">
              CipherBallot
            </span>
          </div>
        </div>
        ...
        {isConnected && (
          <nav className="hidden md:flex items-center gap-1.5 px-1.5 py-1 bg-slate-950/40 rounded-full border border-slate-300/5">
            {navItems.map((item) => (
              <button
                key={item.id}
                ...
                className={`flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-full transition-all duration-200 ${
                  activeTab === item.id
                    ? 'bg-gradient-to-r from-[#FFFFFF] to-[#E2E8F0] text-black shadow-sm shadow-slate-300/10'
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-bold border transition duration-200 ${
                isFheReady
                  ...
                  : isFheInitializing
                  ? 'bg-slate-400/10 text-amber-450 border-slate-400/20 animate-pulse'
          ...
          {/* Wallet Connection */}
          {isConnected ? (
            <div className="flex items-center rounded-full bg-[#141414] border border-slate-300/10 p-1">
              <div className="flex items-center gap-2 px-3 py-1 text-xs text-slate-300 font-semibold">
                <Wallet className="h-3.5 w-3.5 text-[#FFFFFF]" />
                <span className="hidden lg:inline text-slate-400">
                  {chainId.toString() === '31337' ? 'Hardhat' : 'Sepolia'}
                </span>
                <span className="h-1 w-1 rounded-full bg-[#FFFFFF]/30"></span>
                <span className="text-yellow-100">{truncateAddress(address)}</span>
              </div>
              <button
                onClick={disconnectWallet}
                className="rounded-full px-3 py-1 text-xs font-bold text-slate-400 hover:text-white hover:bg-slate-900/20 transition duration-150"
              >
                Disconnect
              </button>
            </div>
      ...
      {isConnected && (
        <nav className="flex md:hidden items-center justify-around border-t border-slate-300/5 bg-black/60 py-2">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={`flex flex-col items-center gap-1 px-3 py-1 text-[10px] font-semibold transition-all duration-200 ${
                activeTab === item.id ? 'text-[#FFFFFF] font-bold' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              {item.icon}
